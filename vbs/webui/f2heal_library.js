...
const MESSAGE_TYPE_GET_VERSION = 18;
const MESSAGE_TYPE_VERSION = 19;
...
class BleManager {
	...
	receiveVolume(messagePayload) {
	this.log("Raw volume payload: " + Array.from(messagePayload));
    // Fix: Access the first element of the Uint8Array correctly
    let value = messagePayload[0];
    this.log("Receive Volume (" + value + ")");
    this.volumeUpdate(value);
	...
	// Request volume after a short delay to ensure settings are processed
    setTimeout(() => {
	...
	receiveStatusBatch(messagePayload) {
		...
		// Start reading firmware_version and default_parameter_settings from the correct offset
		let offset = 13; // offset after battery data (9 + 4 = 13)
		
		// Read firmware_version string
		let fw_len = new DataView(messagePayload.buffer, offset, 1).getUint8(0);  // Get length of firmware version
		offset += 1;  // move offset to the start of the firmware version string
		let fw_bytes = new Uint8Array(messagePayload.buffer, offset, fw_len);  // Extract firmware version bytes
		this.a_firmware = new TextDecoder().decode(fw_bytes);  // Decode it into string
		offset += fw_len;  // move offset after firmware version data
	
		// Read default_parameter_settings string
		let param_len = new DataView(messagePayload.buffer, offset, 1).getUint8(0);  // Get length of default parameters
		offset += 1;  // move offset to the start of the parameter settings string
		let param_bytes = new Uint8Array(messagePayload.buffer, offset, param_len);  // Extract default parameter bytes
		this.a_default_params = new TextDecoder().decode(param_bytes);  // Decode it into string
		offset += param_len;  // move offset after parameter settings data
	
		// Trigger UI update with the new information
		this.onStreamUpdate();
	
		// Log all the received information
		this.log("Status running: " + this.a_running
				+ ", runningsince: " + this.a_runningsince
				+ ", battery: " + this.a_battery
				+ ", firmware: " + this.a_firmware
				+ ", default_params: " + this.a_default_params);
	}

	receiveVersion(messagePayload) {
        // Convert bytes to string and remove control characters
     let versionStr = String.fromCharCode.apply(null, messagePayload).replace(/[^\x20-\x7E]/g, ''); // Remove non-printable ASCII
    this.a_firmware = versionStr;
    this.log("Firmware version: " + versionStr);
    }

    requestFirmwareVersion() {
        if(!this.connected) return;
        this.writeMessage(MESSAGE_TYPE_GET_VERSION, new Uint8Array(0));
    }
	...
	case MESSAGE_TYPE_VERSION:
		this.receiveVersion(messagePayload);
		break;
	default:
