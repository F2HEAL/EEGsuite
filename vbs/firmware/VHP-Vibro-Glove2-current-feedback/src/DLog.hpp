...
public:
    ...
    // Add this function to the public section of DLog
void PrintRawSamples(uint32_t channel) {
	if (channel >= channels_) return;  // Check bounds

	Serial.print("Raw samples for channel ");
	Serial.print(channel);
	Serial.print(" (");
	Serial.print(sample_count_[channel]);
	Serial.println(" samples):");

	for (uint16_t i = 0; i < sample_count_[channel]; i++) {
		Serial.print(i);
		Serial.print(": ");
		Serial.println(logs_[channel][i]);
	}
    }
