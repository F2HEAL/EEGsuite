...
class TFRContrastConfig:
    ...
    stim_freq: float = 84.0            # Hz — must match your VHP protocol
...
class TFRContrastAnalyzer:
    ...
    def run_pipeline(self) -> bool:
        ...
        Execute the 4-step pipeline:
          ...
          4    contrast      → self.tfr_contrast (if both present)
        Returns True on success.
        """
        if self.raw_fot is None:
            logger.error("No data loaded. Call load_two_files() first.")
            return False
        ...
        # Steps 1–3 for IFNFN (if available)
        if self.raw_ifnfn is not None:
        ...
        # Basic validation: must have at least FOT
        if self.tfr_fot is None:
            logger.error("Pipeline failed: could not compute FOT TFR.")
            return False

        # Step 4: Contrast (only if both are available and not identical)
        if self.tfr_ifnfn is not None and self.tfr_ifnfn is not self.tfr_fot:
            ...
            logger.info("Single-condition mode: skipping contrast subtraction.")
    ...
    def plot_stim_band_timecourse(
        ...
        if not np.any(freq_mask):
            logger.warning(
                "Frequency band [%.1f, %.1f] is outside TFR range [%.1f, %.1f] Hz. "
                "Skipping plot for %s.",
                freq_band[0], freq_band[1], freqs.min(), freqs.max(), ch
            )
            return None
...
def main():
    ...
    parser.add_argument(
        "--ifnfn", type=Path, default=None,
        help="CSV file for IFNFN condition (optional)",
    )
    parser.add_argument(
    ...
    if args.ifnfn:
        # Check if it's the same file (interleaved mode) or two separate files
        if args.fot.resolve() == args.ifnfn.resolve():
            analyzer.load_single_file(args.fot)
        else:
            analyzer.load_two_files(args.fot, args.ifnfn)
    else:
        # Load only FOT
        logger.info("Loading single FOT file: %s", args.fot)
        raw = load_csv_to_raw(args.fot, cfg.channels, cfg.montage)
        analyzer._apply_virtual_channels(raw)
        analyzer._apply_picks(raw)
        analyzer.raw_fot = raw
